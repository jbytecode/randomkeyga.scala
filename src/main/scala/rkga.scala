object rkga:
    ...
    case class GAParams(costfn: CostFn, crossprob: Double, mutprob: Double, elitism: Int, maxiter: Int)
    ...
    def one_point_crossover(c1: Chromosome, c2: Chromosome, prob: Double): Chromosome = 
    ...
    def calculate_costs(chromosomes: List[Chromosome], costfn: CostFn): List[Chromosome] = 
    ...
    def generation(pop: Population, params: GAParams): Population = 
        ...
        val sorted = pop.chromosomes.map(c => calculate_cost(c, costfn)).sortBy(_.cost)
    ...
    def ga(psize: Int, chsize: Int, costfn: CostFn, crossprob: Double, mutprob: Double, elitism: Int, maxiter: Int): Population =
